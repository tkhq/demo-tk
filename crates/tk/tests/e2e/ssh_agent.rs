...
impl<'r> Agent<'r> {
    ...
    /// The `ssh-ed25519 <base64>` lines the agent advertises, sorted.
    fn listed_keys(&self, ssh_add: &Path) -> Vec<String> {
        ...
        let mut lines: Vec<String> = String::from_utf8_lossy(&listed.stdout)
            ...
            .map(|line| {
                ...
                assert!(
                    comment.starts_with("turnkey:"),
                    "the agent comment should name the Turnkey key: {line}"
                );
...
fn line(registered: &Value) -> String {
    text(&registered["publicKey"]).to_string()
}
...
fn agent_serves_every_registered_key_and_reports_its_lifecycle() {
    ...
    assert_eq!(
        ...
        sorted(vec![line(&first), line(&second)])
    );
    ...
    // says so.
    let mut readd = bare_cli(run.home());
    for (name, value) in run.admin().get_envs() {
        match value {
            Some(value) => readd.env(name, value),
            None => readd.env_remove(name),
        };
    }
    let readded = run.human_stdout(
        readd
            .arg("--api-base-url")
            .arg(&run.config.api_base_url)
            .args(["ssh", "keys", "add", "--private-key-id", &first_id]),
    );
    assert!(
        readded
            .trim_end()
            .ends_with("; restart tk ssh agent to pick this up"),
        "{readded}"
    );
    ...
    assert_eq!(restarted.listed_keys(&ssh_add), vec![line(&second)]);
...
fn agent_start_narrows_by_key_profile_and_organization() {
    ...
    assert_eq!(narrowed.listed_keys(&ssh_add), vec![line(&first)]);
