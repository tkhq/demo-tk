...
fn a_key_whose_organization_has_no_profile_names_the_missing_login() {
    assert_shim_failure(|home| {
        ...
        (
            cmd,
            format!(
                r#"error: select a credential for OpenPGP key {FINGERPRINT}: no profile holds a credential for organization {KEY_ORG}; run tk profile create <name> --organization-id {KEY_ORG}
"#
            ),
        )
    });
