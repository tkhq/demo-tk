//! Versioned framing for `OpenPGP` agent requests and responses.

use std::io;
use std::str::from_utf8;

use tokio::io::{AsyncRead, AsyncReadExt, AsyncWrite, AsyncWriteExt};
use turnkey_auth::openpgp::entity::ArmoredSignature;
...
pub(in crate::gpg) const MAX_PAYLOAD_LEN: usize = 1024 * 1024;
    ...
    T: TryFrom<usize>,
{
    ...
    T::try_from(len).map_err(|_error| io::Error::new(io::ErrorKind::InvalidInput, message))
}
...
"#;
    ...
    #[tokio::test]
    async fn rejects_oversized_requests_without_writing() {
        let oversized_key = "A".repeat(MAX_KEY_LEN + 1).parse().unwrap();
        let oversized_payload = vec![0_u8; MAX_PAYLOAD_LEN + 1];

        for (key, payload, message) in [
            (Some(&oversized_key), &[][..], "signing key is too long"),
            (None, oversized_payload.as_slice(), "payload is too large"),
        ] {
            let mut output = Vec::new();
            let error = write_request(&mut output, key, payload).await.unwrap_err();
