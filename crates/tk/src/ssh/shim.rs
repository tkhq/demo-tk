...
use crate::ssh::{client_for_entry, selection_error, signer::TurnkeySigner};
...
pub enum Invocation {
    ...
    /// An ssh-keygen verification operation.
    Passthrough,
}
...
pub async fn sign(args: &[String], options: &AuthOptions) -> Result<()> {
    match Invocation::parse(args)? {
        Invocation::Sign {
            public_key_path,
            payload_path,
        } => sign_paths(public_key_path, payload_path, options).await,
        Invocation::Passthrough => Err(InvalidInput(
...
async fn sign_paths(
    ...
    let entry = auth::load_ssh_keys(options)
        .await?
        .select(Some(SshKeyName::PublicKey(key)))
        .map_err(git_selection_error)?;
    let (client, organization_id) = client_for_entry(options, &entry).await?;
    let signed_data = build_signed_data("git", &payload);
    let signature = TurnkeySigner::new(&client, organization_id.to_string(), &entry.private_key_id)
...
fn git_selection_error(error: SelectError) -> anyhow::Error {
    match &error {
        ...
        SelectError::Unnamed { .. } | SelectError::NoMatch { .. } | SelectError::Ambiguous { .. } => selection_error(error, "set user.signingkey to a registered key"),
    }
}
...
mod tests {
    ...
    fn parses_git_sign_with_and_without_public_key_flag() {
        for invocation in [
            ...
            match Invocation::parse(&invocation).expect("git's sign call should parse") {
                ...
                Invocation::Passthrough => panic!("a sign call must not be passed through"),
            }
        }
    }
    ...
    fn recognizes_ssh_keygen_verification_operations() {
        for operation in ["verify", "find-principals", "check-novalidate"] {
            assert!(matches!(
                Invocation::parse(&args(&["-Y", operation])),
    ...
    #[test]
    fn rejects_non_git_namespaces() {
        let error = Invocation::parse(&args(&[
            "-Y", "sign", "-n", "file", "-f", "key.pub", "payload",
        ]))
        .err()
        .expect("a non-git namespace should fail");

        assert_eq!(error.to_string(), "unsupported SSH signing namespace: file");
    }
