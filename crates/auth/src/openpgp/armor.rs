...
impl ArmoredSignature {
    /// Parses and armors a detached signature packet.
    pub fn from_packet(packet: &[u8]) -> Result<Self, OpenPgpError> {
        let parsed =
            DetachedSignaturePacket::parse(packet).ok_or(OpenPgpError::InvalidSignatureArmor)?;
        Ok(Self {
...
mod tests {
    ...
    fn parses_a_constrained_signature_and_retains_its_hashed_metadata() {
        ...
        assert_eq!(ArmoredSignature::from_packet(&packet()).unwrap(), signature);
    }
    ...
    fn rejects_wrong_or_inconsistent_subpackets() {
        ...
        for bytes in cases {
            assert!(matches!(
                ArmoredSignature::from_packet(&bytes),
                Err(OpenPgpError::InvalidSignatureArmor)
            ));
        }
    }
    ...
    fn accepts_p256_curve_order_minus_one_in_either_scalar() {
        ...
        for bytes in [
            ...
            ArmoredSignature::from_packet(&bytes).unwrap();
