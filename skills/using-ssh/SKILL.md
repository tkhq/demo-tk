...
1. **Create the private key.** Root, once:
   ...
   `ssh_key_created` carries `fingerprint`, `publicKey`, `organizationId`,
   and `privateKeyId`. Save `privateKeyId` as `PRIVATE_KEY_ID`; the policy
   and the agent host both need it. Rerunning with the same name registers
...
3. **Register the key and publish the public half.** The agent, in the home
   ...
   `ssh_key_registered` carries `fingerprint`, `publicKey`,
   `organizationId`, and `privateKeyId`; re-adding the same id overwrites
   the entry. Install `agent-ssh.pub` where the server expects it: the
...
4. **Start the agent.** The agent identity, with `--profile` explicit:
   ...
   serves; `agent_status_report` repeats them while it runs. Pass `--socket`
   and `--pid-file` to move both away from the defaults, and the same flags
   to `status` and `stop`. Add `--key SSH_FINGERPRINT` to serve one key when
...
5. **Point clients at the socket and verify.** Per repository, or per
   ...
   ```sh
   git config core.sshCommand "ssh -o IdentityAgent=~/.config/turnkey/ssh-agent.sock"
   ssh -o IdentityAgent=~/.config/turnkey/ssh-agent.sock user@host
   ```
