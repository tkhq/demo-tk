...
- [git-signing](../../docs/git-signing.md): git configuration for both
  signing programs and the verification environment variables.
- [gpg-signing](../../docs/gpg-signing.md): `gpg` command records, key
  management, and how git's GPG calls reach `tk`.
...
- `git verify-commit` fails after a successful commit: for GPG, the public
  key is not in the `GNUPGHOME` git reads, or its ownertrust was not
  imported; for SSH, the `allowed_signers` line does not pair the committer
  email with the signing public key, or `gpg.ssh.allowedSignersFile` is
  unset.
...
- [sidecar-patterns](../sidecar-patterns/SKILL.md): where `HOME` and
  `TK_PROFILE` are pinned for a process that runs git.
