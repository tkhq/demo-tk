...
for tool in awk curl install mkdir mktemp mv rm tar uname; do
...
curl --proto '=https' --tlsv1.2 -LsSf --fail-early \
    -o "$archive" "$download_url/$archive_name" \
    -o "$checksum" "$download_url/$checksum_name"
